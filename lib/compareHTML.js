...
function checkStyle(current, original) {
  const currentStyles = getStyles(current);
  const originalStyles = getStyles(original);
  for (let index = 0; index < currentStyles.length; index++) {
    const cssResult = compareCSS(currentStyles[index].text, originalStyles[index].text, false);
    if (!cssResult.result) {
      for (let errorIndex = 0; errorIndex < cssResult.errors.length; errorIndex++) {
        if (cssResult.errors[errorIndex].line) {
          cssResult.errors[errorIndex].line += currentStyles[index].line - 1;
        }
      }
      return cssResult;
    }
...
module.exports = function (current, original) {
  ...
  const styleResult = checkStyle(current, original);
